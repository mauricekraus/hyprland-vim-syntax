...
syn match    HyprVar '\s[a-z _ .]\+ ' skipwhite contained display nextgroup=Symbol,HyprDescMonitor
...
" Commands
syn region   Command start='^[a-zA-Z][a-zA-Z_. -]\+ =' end='$' skipwhite transparent contains=HyprKeyModifier,ShellVar,HyprConfigCommand,Dispatchers,HyprComment,Str,Disp,Path,Num,HyprDescMonitor
...
" Highlight 'desc:monitor' along with any following text
syn region   HyprDescMonitor start='desc:monitor:' end='\s\+[^=]\+' contained

" Highlight 'default:true' in two different colors
syn region   DefaultKeyword start='default:' end='true' contained
syn keyword  BooleanTrue true contained

" Link the new syntax groups to their highlight colors
...
highlight link DefaultKeyword         Constant
highlight link BooleanTrue            Boolean
